...
if [ -n "$MYSQLHOST" ] && [ -n "$MYSQLUSER" ] && [ -n "$MYSQLDATABASE" ]; then\n\
  ...
  mysql -h $MYSQLHOST -u $MYSQLUSER -p$MYSQLPASSWORD -P $MYSQLPORT --ssl-mode=DISABLED $MYSQLDATABASE < schema.sql && echo "Database initialized successfully!" || echo "Warning: Database setup failed or already initialized"\n\
else\n\
