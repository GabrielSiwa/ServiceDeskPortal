...
if [ -n "$MYSQLHOST" ] && [ -n "$MYSQLUSER" ] && [ -n "$MYSQLDATABASE" ]; then\n\
  ...
  mysql -h $MYSQLHOST -u $MYSQLUSER -p$MYSQLPASSWORD $MYSQLDATABASE --ssl-mode=SKIP-VERIFY < schema.sql && echo "Database initialized successfully!" || echo "Warning: Database setup failed or already initialized"\n\
else\n\
